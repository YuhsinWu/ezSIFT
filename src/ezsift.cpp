                         ...
                         int gR)
{
    ...
    for (int r = 0; r < h; r++) {
        ...
        for (int i = 0; i < gL; i++) {
            ...
            //     dstData ++;
            // }
            __m128 m128_kernel_element = _mm_set_ps1(*coef);

            for (int j = 0; j < steps; j++) {
                __m128 m_temp0, m_temp1, m_temp2, m_temp3;

                m_temp0 = _mm_loadu_ps(prow);
                m_temp1 = _mm_mul_ps(m_temp0, m128_kernel_element);
                m_temp2 = _mm_loadu_ps(dstData);
                m_temp3 = _mm_add_ps(m_temp1, m_temp2);
                _mm_storeu_ps(dstData, m_temp3);
                prow += step_size;
                dstData += step_size;
            }/*for sse*/

            for (int j = steps*step_size; j < w; j++) {
                *dstData += *prow * (*coef);
                prow += 1;
                dstData += 1;
            }/*remainder*/
...
int col_filter(const float *src, float *dst, int w, int h, float *coef1d,
                         int gR)
{
    // float *row_buf = new float[w + gR * 2];
    // float *row_start;
    ...
    const float * __restrict srcData = src;
    float* __restrict dstData;
    // float partialSum = 0.0f;
    float* __restrict coef = coef1d;
    // float *prow;
    ...
    for(int i=0;i<gR;i++){
        ...
        memcpy(p_exsrc + offset, srcData, w*sizeof(float));
        p_exsrc+=w;
    }
...
int row_filter_avx(float *src, float *dst, int w, int h, float *coef1d,
                         int gR)
{
    float *row_buf = new float[w + gR * 2];
    float *row_start;
    ...
    int step_size = sizeof(__m256) / sizeof(float);
    int steps = w / step_size;
    ...
    for (int r = 0; r < h; r++) {
        ...
        for (int i = 0; i < gL; i++) {
            ...
            //     dstData ++;
            // }
            __m256 m256_kernel_element = _mm256_set1_ps(*coef);

            for (int j = 0; j < steps; j++) {
                __m256 m_temp0, m_temp1, m_temp2, m_temp3;

                m_temp0 = _mm256_loadu_ps(prow);
                m_temp1 = _mm256_mul_ps(m_temp0, m256_kernel_element);
                m_temp2 = _mm256_loadu_ps(dstData);
                m_temp3 = _mm256_add_ps(m_temp1, m_temp2);
                _mm256_storeu_ps(dstData, m_temp3);
                prow += step_size;
                dstData += step_size;
            }/*for sse*/
...
int col_filter_avx(const float *src, float *dst, int w, int h, float *coef1d,
                         int gR)
{
    // float *row_buf = new float[w + gR * 2];
    // float *row_start;
    ...
    for (int r = 0; r < h; r++) {
        ...
        for (int i = 0; i < gL; i++) {
            ...
            for (int j = 0; j < steps; j++) {
                ...
                m_temp0 = _mm256_loadu_ps(p_exsrc);
                m_temp1 = _mm256_mul_ps(m_temp0, m256_kernel_element);
                  ...
                  std::vector<float> coef1d)
{
    ...
    // Image<float> img_t1(h, w);
    // Image<float> out_image1(h, w);
    // row_filter_transpose(in_image.data, img_t1.data, w, h, &coef1d[0], gR);
    // row_filter_transpose(img_t1.data, out_image1.data, h, w, &coef1d[0], gR);

    Image<float> img_t(w, h);
    row_filter_avx(in_image.data, img_t.data, w, h, &coef1d[0], gR);
    col_filter_avx(img_t.data, out_image.data, w, h, &coef1d[0], gR);
    return 0;
}
